...
const EditMenu = () => {
  const { axios, navigate, loading, setLoading, categories,menus } =
...
menuData.append("description", formData.description);
menuData.append("category", formData.category);

if (formData.image) {
  menuData.append("image", formData.image);
}
    const { data } = await axios.put(`/api/menu/update/${id}`,menuData,
    {
    headers: {
        ...
        <div>
          ...
          >
            ...
            <span className="text-gray-600 text-sm">
              {file ? file.name : "current image selected"}
            </span>
          </label>
        </div>
